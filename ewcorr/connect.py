"""Native cognis-connect emit for ewcorr — forward findings to any platform.

Maps ewcorr's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ewcorr ... --format json | ewcorr-emit --to stix
    ewcorr-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('emitter_id', '')
        out['severity'] = 'medium'
        out['type'] = 'emitter'
        out['description'] = f"Emitted at {rec.get('freq_center_mhz', 0):.2f} MHz"
        out['tags'] = ['ew', 'emitter']
